...
to setup-cars
  ...
  create-turtles number-of-cars [
    ...
    set heading 90
    ;; set initial speed to be in range 0.1 to 1.0 (ORIGINAL)
    set speed-limit 0.5 + random-float 0.5 ;;Set individual variable speed-limit for each car
    set speed ((0.1 + random-float 0.9) * (speed-limit)) ;; Set starting speed as fraction of speed-limit
    separate-cars
  ]
...
to go
  ...
  ask turtles [
    ...
    ifelse car-ahead != nobody
      [
      set desired-distance 1 + speed * reaction-time + ( [ speed ] of car-ahead ) * ( [ speed ] of car-ahead ) / (2 * acceleration)
      let actual-distance [distance myself] of car-ahead
      show actual-distance
      ifelse desired-distance >= actual-distance
