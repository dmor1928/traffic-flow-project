...
num_ticks_limit = 50_000  # 1_000_000
...
# Changing variables
vertical_pixels = 251
horizontal_pixels = 251
width_N_vals = np.linspace(85., 135., num=vertical_pixels, endpoint=True)  # width / number_of_cars dimensionless
print(width_N_vals)
dec_acc_vals = np.linspace(100, 200., num=horizontal_pixels, endpoint=True)   # dec / acc dimensionless
print(dec_acc_vals)
...
for i, width_N in enumerate(width_N_vals):
    ...
    for j, dec_acc in enumerate(dec_acc_vals):
        ...
        for run in range(num_runs):
            ...
            all_pos_data, avg_v, num_ticks_new, final_state_run_info, final_x, final_v = run_sim(num_ticks, sample_mod, init_x, init_v, dec, width, min_delta)
...
np.savez("databank/09-02-251x251_100N_seed42v_eqspace_100to200da_50K",
