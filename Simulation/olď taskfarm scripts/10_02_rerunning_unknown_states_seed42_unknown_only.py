...
def cross_correlate_pairs(cars):  # Finds the time lag and avg x separation from the car in front, for each car.
    ...
    for n, car in enumerate(cars):
        ...
        only_pos = [num_ticks - cut_off, num_ticks - cut_off + width]  # Only returns the +ve displacement and obviously < width
        correlation = signal.correlate(car_x - trans_zero, leading_x - trans_zero)[only_pos[0]:only_pos[1]]
    ...
    while running:
        ...
        if tick >= num_ticks_new:
            ...
            final_state_information = np.array([1, earliest_tick, 1, soliton_count, soliton_init_x, soliton_period_est,
                                                0, 0], dtype=int)
            return all_pos_data, average_v, num_ticks_new, final_state_information, x, v  #, earliest_x_position, soliton_period_est
        ...
        if tick == num_ticks:  # Reached end of simulation
            if final_state_reached is True:
                ...
                final_state_information = np.array([1, earliest_tick, 1, soliton_count, soliton_init_x, soliton_period_est,
                                                    0, 0], dtype=int)
                return all_pos_data, average_v, num_ticks, final_state_information, x, v
            else:
                # print(f"extra_time: {extra_time}")
                ...
                return all_pos_data, average_v, num_ticks, final_state_information, x, v  # Returns cut_off value = 0.
        ...
        if np.all(v == 1.):  # Optimisation makes free-flow runs end early.
            ...
            return all_pos_data, average_v, num_ticks, final_state_information, x, v  # Returns cut_off value of where free flow was reached.
...
file_name = "09-02-251x251_100N_seed42v_eqspace_100to200da_50K.npz"
file = np.load(file_name)

initial_parameters = file['init_car_params'].copy()
final_parameters = file['final_car_params'].copy()
final_state_analysis = file['final_param'].copy()
system_parameters = file['sys_param'].copy()
final_state_information = file['final_info'].copy()

vertical_pixels = initial_parameters.shape[0]
horizontal_pixels = initial_parameters.shape[1]
num_runs = initial_parameters.shape[2]

max_v = 1.0
cut_off = 0  # (initialise global variable)
previous_num_ticks_limit = 50_0000
num_ticks_limit = 100_000  # So total num_ticks_limit is 150_000 ticks
for i in range(vertical_pixels):
    for j in range(horizontal_pixels):
        for run in range(num_runs):
            ticks_saved = int(final_state_analysis[i, j, run, -1])
            if (final_state_information[i, j, run, 2] == 2.0):

                # print(f"running index [{i}, {j}, {run}]")

                init_x, init_v = final_parameters[i, j, run]

                number_of_cars = int(system_parameters[i, j, 0])
                width = int(system_parameters[i, j, 1])
                acc = system_parameters[i, j, 2]
                dec = system_parameters[i, j, 3]
                min_delta = system_parameters[i, j, 4]

                previous_num_ticks = final_state_information[i, j, run, 1]

                all_pos_data, avg_v, num_ticks_new, final_state_run_info, final_x, final_v = run_sim(num_ticks_limit, 1, init_x, init_v, dec, width, min_delta)
                num_ticks = num_ticks_new
                cut_off = final_state_run_info[1]  # = time to reach final state
                final_state_type = int(final_state_run_info[2])
                ...
                final_state_run_info[1] += previous_num_ticks  # Add previous run num_ticks
                final_parameters[i, j, run] = np.asarray([final_x, final_v])
                final_state_information[i, j, run] = final_state_run_info  # Saves final_state_info from run_sim

np.savez("databank/"+file_name[0:-4]+"+100K_unknown_only",
