    ...
    while running:
        ...
        if tick == num_ticks:  # Reached end of simulation
            if final_state_reached is True:
                ...
                #                             soliton_count (0 def), soliton_init_x (0 def), soliton_period_est (0 def),
                #                             ff_jump? (1/0), ff_jump_tick (0 def)]
                final_state_information = np.array([0, 0, 2, 0, 0, 0, 0, 0], dtype=int)
        ...
        if np.all(v == 1.):  # Optimisation makes free-flow runs end early.
            remaining_pos_data = x.repeat((int(num_ticks / sample_modulus) - sample_index))
            ...
            average_v = np.concatenate((average_v[:sample_index], np.repeat(1., int(num_ticks / sample_modulus) - sample_index)))
...
initial_parameters = np.empty((201, 201, num_runs, 2, number_of_cars))
system_parameters = np.empty((201, 201, 5))
final_state_analysis = np.empty((201, 201, num_runs, 11))
final_state_information = np.empty((201, 201, num_runs, 8))
