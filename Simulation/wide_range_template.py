    ...
    while running:
        ...
        if tick == 5_000 + extra_time * 5_000:  #Checks for soliton peaks, every 5_000 ticks
            ...
            if detection_results == (0, 0, 0, 0):
                ...
                print(f"soliton count: {soliton_count}")
        ...
        if tick == num_ticks_lim:  # Reached end of simulation
            if final_state_reached is True:
                ...
                final_state_information = np.array([1, earliest_tick, 1, soliton_count, soliton_init_x, soliton_period_est,
                                                    0, 0], dtype=int)
                interactions = interactions[0:int(num_ticks_lim / sample_modulus), :]
...
mean_interaction_length = np.empty((vertical_pixels, horizontal_pixels, num_runs))
active_interactions_per_tick = np.empty((vertical_pixels, horizontal_pixels, num_runs, number_of_cars))
...
for i, width_N in enumerate(width_N_vals):
    ...
    for j, dec_acc in enumerate(dec_acc_vals):
        ...
        for run in range(num_runs):
            ...
            if final_state_run_info[2] != 0.0:  # If not free flow (so if soliton of unknown)

                if final_state_run_info[2] == 2.0:  # If unknown, set cut_off to zero
                    interactions_cut_off = 0
                else:                               # If soliton, cut off ticks before final state reached
                    interactions_cut_off = final_state_run_info[1]

                mean_interactions_lengths = np.empty(number_of_cars)  # Stores the mean interaction time, per car

                for c_index, c in enumerate(interactions[interactions_cut_off:].T):
                    interactions_lengths = np.zeros(num_ticks - int(interactions_cut_off))  # Stores interaction lengths for car c
                    k = 0  # Index, per interaction
                    tick = 0
                    while tick < num_ticks - interactions_cut_off:
                        if c[tick] == 1.0:  # Start of interaction
                            count = 1
                            while tick < num_ticks - interactions_cut_off - 1 and c[tick+1] == 1.0:
                                tick += 1
                                count += 1  # Counts time length of interaction
                            interactions_lengths[k] = count  # Stores interaction lengths for a single car c
                            k += 1
                        tick += 1
                    mean_interactions_lengths[c_index] = np.mean(interactions_lengths[0:k])  # Stores mean interaction length, for every car
                mean_interaction_length_run = np.mean(mean_interactions_lengths)  # Average length of interactions for every car

            else:  # If free flow (no interactions after reached final state)
                mean_interaction_length_run = 0
                interactions_cut_off = final_state_run_info[1]  # Assigned ready for number of interactions count.

            mean_interaction_length[i, j, run] = mean_interaction_length_run

            active_interactions_tally = np.zeros(number_of_cars)  # Tallying number of ticks
            num_interactions_per_tick = np.count_nonzero(interactions[interactions_cut_off:], axis=1)
            active_interactions_tally_up2 = np.stack((np.unique(num_interactions_per_tick, return_counts=True)), axis=-1).T[1]
            max_active_interactions = len(active_interactions_tally_up2) - 1
            active_interactions_tally[:max_active_interactions + 1] = active_interactions_tally_up2
            active_interactions_per_tick[i, j, run] = active_interactions_tally
...
np.savez("databank/split_SPLIT_NUM",
         ...
         interactions_count=active_interactions_per_tick,
         interaction_length=mean_interaction_length)
