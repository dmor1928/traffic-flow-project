...
def run_sim(num_ticks, sample_modulus, initial_x, initial_v, dec, width, min_delta):
    ...
    while running:
        ...
        if tick == 5_000 + extra_time * 5_000:  # Checks for soliton peaks, every 5_000 ticks
            ...
            earliest_tick, soliton_init_x, soliton_period_est, soliton_count = soliton_final_state_detection(
                cars_final_state_det)
            if (earliest_tick, soliton_init_x, soliton_period_est, soliton_count) == (0, 0, 0, 0):
                ...
                all_pos_data = all_pos_data[0:int(num_ticks_new / sample_modulus),
                               :]  # Cut all_pos_data to new tick shape
        ...
        if tick == num_ticks:  # Reached end of simulation
            if final_state_reached is True:
                ...
                final_state_information = np.array(
                    [1, earliest_tick, 1, soliton_count, soliton_init_x, soliton_period_est,
                     0, 0], dtype=int)
        ...
        if np.all(v == 1.):  # Optimisation makes free-flow runs end early.
            ...
            remaining_pos_data = np.reshape(remaining_pos_data,
                                            (int(num_ticks / sample_modulus) - sample_index, number_of_cars))
            all_pos_data = np.concatenate((all_pos_data[:sample_index], remaining_pos_data))
            # all_pos_data[sample_index:] = remaining_pos_data
            average_v = np.concatenate(
                (average_v[:sample_index], np.repeat(1., int(num_ticks / sample_modulus) - sample_index)), axis=None)
            ...
            final_state_information = np.array([1, tick, 0, soliton_count, soliton_init_x, soliton_period_est, 0, 0],
                                               dtype=int)
...
def run_plots(cut_off):
    ...
    num_time_points = 500
    resolution = 10
    time_points = np.linspace(0, int(soliton_period_est * 10), num=num_time_points, dtype='int') + final_state_tick
    colors = plt.cm.viridis(np.linspace(0, 1, num=num_time_points))
    for car_sigma in [0.5, 1, 1.5, 2.0]:
        fig11, ax11 = plt.subplots()

        gaussian_window_len = int(10 * min_delta * resolution * car_sigma)  # so 10 sigmas fit in each window
        for k, t in enumerate(time_points):
            ...
            for point in x:
                ...
                print(len(gaussian_density))
                print(int(point * resolution))
                if (int(point * resolution) < gaussian_window_len // 2) :
                    gaussian_density[:int(point * resolution) + gaussian_window_len // 2] += point_gaussian[-(int(point * resolution) + gaussian_window_len // 2):]
                    gaussian_density[-(gaussian_window_len // 2 - int(point * resolution)):] += point_gaussian[:gaussian_window_len // 2 - int(point * resolution)]
                if int(point * resolution) > gaussian_window_len // 2 and int(point * resolution) < int(width*resolution) - gaussian_window_len // 2:
                    gaussian_density[int(point * resolution - 0.5 * gaussian_window_len): int(point * resolution + 0.5 * gaussian_window_len)] += point_gaussian
                # if int(point * resolution) > int(width * resolution) - gaussian_window_len // 2:
                #     gaussian_density[-(int((width - point) * resolution) + gaussian_window_len // 2):] += point_gaussian[int((width - point) * resolution) + gaussian_window_len // 2:]
                #     gaussian_density[:gaussian_window_len // 2 - int((width - point) * resolution)] += point_gaussian[gaussian_window_len // 2 + int((width - point) * resolution)]
            # # ax11.plot(dirac_density)
            # smooth_density = gaussian_filter1d(dirac_density, int(car_sigma * 100), mode='wrap')
            # translation += int(10 * (width // soliton_period_est) * (soliton_period_est // 20))
        ...
        ax11.set_xticks(np.linspace(0, int(width * 10), num=9))
...
dec_acc_vals = np.linspace(80, 81, num=1, endpoint=True)   # dec / acc dimensionless
