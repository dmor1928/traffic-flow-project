...
def run_plots(cut_off):
    ...
    lc = LineCollection(segments, cmap='viridis_r', norm=norm, zorder=1)
    ...
    ax10.scatter(x=cars[car_index].X[:final_state_tick][[0, -1]], y=cars[car_index].V[:final_state_tick][[0, -1]],
    ...
    ax10.grid()
    ax10.set_xlim(cars[car_index].X.min() - 5, cars[car_index].X.max() + 5)
...
dec_acc_vals = np.linspace(20, 21, num=1, endpoint=True)   # dec / acc dimensionless
