...
def run_plots(cut_off):
    ...
    for car_index in [5]:
        ...
        if np.min(cars[car_index].X - np.mean(cars[car_index].X[cut_off:])) < overall_min_x:
            overall_min_x = np.min(cars[car_index].X - np.mean(cars[car_index].X[cut_off:]))

        if np.max(cars[car_index].X - np.mean(cars[car_index].X[cut_off:])) > overall_max_x:
            overall_max_x = np.max(cars[car_index].X - np.mean(cars[car_index].X[cut_off:]))

        # Plotting final state point(s)
        if final_state_type == 1.0:
            ax10.plot(cars[car_index].X[final_state_tick:final_state_tick + soliton_period_est] - np.mean(cars[car_index].X[cut_off:]),
                      cars[car_index].V[final_state_tick:final_state_tick + soliton_period_est],
                      c='k', linewidth=1.5, zorder=0)
        if final_state_type == 0.0:
            ax10.scatter(x=cars[car_index].X[:final_state_tick][-1] - np.mean(cars[car_index].X[cut_off:]),
                         y=cars[car_index].V[:final_state_tick][-1], linewidths=3, c='k')

        points = np.array([cars[car_index].X[:final_state_tick] - np.mean(cars[car_index].X[cut_off:]), cars[car_index].V[:final_state_tick]]).T.reshape(-1, 1, 2)
        segments = np.concatenate([points[:-1], points[1:]], axis=1)
        # Create a continuous norm to map from data points to colors
        lc = LineCollection(segments, cmap='viridis_r', norm=norm, linewidth=1, zorder=1)
        ...
        ax10.scatter(x=cars[car_index].X[:final_state_tick][[0, -1]] - np.mean(cars[car_index].X[cut_off:]),
                     y=cars[car_index].V[:final_state_tick][[0, -1]],
                     linewidths=0.5, c=[0, 1], cmap=my_cmap)
        ax10.arrow(cars[car_index].X[final_state_tick // 2] - np.mean(cars[car_index].X[cut_off:]),
        ...
        ax10.arrow(cars[car_index].X[final_state_tick // 4] - np.mean(cars[car_index].X[cut_off:]),
        ...
        ax10.arrow(cars[car_index].X[final_state_tick - final_state_tick // 4] - np.mean(cars[car_index].X[cut_off:]),
...
number_of_cars = 30  # Might be changing variable in future?
