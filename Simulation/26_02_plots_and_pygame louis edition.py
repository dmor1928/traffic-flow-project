...
def run_analysis(final_state_information):
    ...
    superperiod = int(final_state_information[5])
    print("FINAL STATE INFORMATION[5] IS ",final_state_information[5])
...
dec_acc_vals = np.linspace(100, 101, num=horizontal_pixels, endpoint=True)   # dec / acc dimensionless
...
for i, width_N in enumerate(width_N_vals):
    ...
    for j, dec_acc in enumerate(dec_acc_vals):
        ...
        for run in range(num_runs):
            ...
            #see where soliton was first detected


            #pull interaction index at this point in time
            int_array_first = interactions[supercutoff]

            #find cars that are interacting at this point in time
            int_array_start_index = []
            for i in range(len(int_array_first)):
                if int_array_first[i] == 1:
                    int_array_start_index.append(i)

            #track car that is at the front of the soliton
            for i in int_array_start_index:
                indexy = i
                int_indexes = []
                int_switches = []
                for t in range(num_ticks - supercutoff):
                    interaction_check_left = interactions[supercutoff + t][indexy - 1]
                    if interaction_check_left == 1:
                        indexy = (indexy - 1) % number_of_cars
                        int_switches.append(t)
                    int_indexes.append(indexy)
                int_superindex.append(int_indexes)
                int_switchindex.append((int_switches))

            soliton_superspeedy = []


            for i in range(len(int_superindex)):
                soliton_speedy = []
                for j in range(len(int_superindex[i])):
                    soliton_speedy.append(cars[int_superindex[i][j]][1][supercutoff + j])
                # print(f"int_superindex[{i}]={int_superindex[i]}")
                soliton_superspeedy.append(soliton_speedy)
                #print(soliton_speedy)
                plt.plot(soliton_speedy[20000:], c='blue',label="Instantaneous speed")

                box = np.ones(33772) / 33772
                box_smooth = np.convolve(soliton_speedy[20000:], box, mode='same')
                plt.plot(box_smooth, c="red", label="Average speed measured over 1 soliton period")

                plt.title("Plot of the instantaneous speed of the vehicle \n carrying the front of the soliton over time")
                plt.xlabel("Time from soliton detection")
                plt.ylabel("Speed v")
                plt.ylim(-1.0, 0.0)
                plt.legend()
                plt.show()
